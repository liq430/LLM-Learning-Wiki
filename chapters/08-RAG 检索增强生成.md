...
- [8.0 本章知识地图](#80-本章知识地图)
- [8.1 RAG 全景](#81-rag-全景)
  - [8.1.1 为什么需要 RAG](#811-为什么需要-rag)
  - [8.1.2 RAG vs 微调：什么时候选谁](#812-rag-vs-微调什么时候选谁)
  - [8.1.3 RAG 的三种范式：Naive / Advanced / Modular](#813-rag-的三种范式naive--advanced--modular)
  - [8.1.4 RAG 与长上下文：会不会被替代](#814-rag-与长上下文会不会被替代)
- [8.2 文档解析](#82-文档解析)
  - [8.2.1 PDF 的坑](#821-pdf-的坑)
  - [8.2.2 主流工具对比](#822-主流工具对比)
  - [8.2.3 版面分析与阅读顺序](#823-版面分析与阅读顺序)
  - [8.2.4 表格与公式](#824-表格与公式)
  - [8.2.5 解析质量的下游影响](#825-解析质量的下游影响)
- [8.3 切分 Chunking](#83-切分-chunking)
  - [8.3.1 固定长度切分](#831-固定长度切分)
  - [8.3.2 递归字符切分（RecursiveCharacterTextSplitter）](#832-递归字符切分recursivecharactertextsplitter)
  - [8.3.3 语义切分（Semantic Chunking）](#833-语义切分semantic-chunking)
  - [8.3.4 基于文档结构（Markdown 标题树 / 父子块）](#834-基于文档结构markdown-标题树--父子块)
  - [8.3.5 Late Chunking（Jina AI 2024）](#835-late-chunkingjina-ai-2024)
  - [8.3.6 Small-to-Big 与句子窗口检索](#836-small-to-big-与句子窗口检索)
  - [8.3.7 块大小怎么选](#837-块大小怎么选)
  - [8.3.8 切分评估](#838-切分评估)
- [8.4 Embedding 模型](#84-embedding-模型)
  - [8.4.1 演进时间线](#841-演进时间线)
  - [8.4.2 Sentence-BERT 原理（必须吃透）](#842-sentence-bert-原理必须吃透)
  - [8.4.3 主流模型速查（2025-2026）](#843-主流模型速查2025-2026)
  - [8.4.4 MTEB / C-MTEB 榜单怎么看](#844-mteb--c-mteb-榜单怎么看)
  - [8.4.5 Matryoshka 套娃表示学习](#845-matryoshka-套娃表示学习)
  - [8.4.6 指令前缀（asymmetric retrieval）](#846-指令前缀asymmetric-retrieval)
  - [8.4.7 向量维度、归一化与距离](#847-向量维度归一化与距离)
- [8.5 向量数据库](#85-向量数据库)
  - [8.5.1 ANN 三大类索引](#851-ann-三大类索引)
  - [8.5.2 HNSW 详解（Hierarchical Navigable Small World）](#852-hnsw-详解hierarchical-navigable-small-world)
  - [8.5.3 量化（PQ / SQ / IVF-PQ）](#853-量化pq--sq--ivf-pq)
  - [8.5.4 各向量库 / 引擎对比](#854-各向量库--引擎对比)
- [8.6 混合检索与重排](#86-混合检索与重排)
  - [8.6.1 BM25（Best Matching 25）](#861-bm25best-matching-25)
  - [8.6.2 SPLADE（学出来的稀疏向量）](#862-splade学出来的稀疏向量)
  - [8.6.3 RRF：倒数排序融合（Reciprocal Rank Fusion）](#863-rrf倒数排序融合reciprocal-rank-fusion)
  - [8.6.4 Rerank 重排](#864-rerank-重排)
- [8.7 Query 侧优化](#87-query-侧优化)
  - [8.7.1 Query 改写（Rewrite）](#871-query-改写rewrite)
  - [8.7.2 HyDE（Hypothetical Document Embeddings）](#872-hydehypothetical-document-embeddings)
  - [8.7.3 Multi-Query 多路查询](#873-multi-query-多路查询)
  - [8.7.4 Step-Back Prompting](#874-step-back-prompting)
...
以 Claude 3.5 / Gemini 1.5 / GPT-4o（2024 年代际）为起点，主流闭源模型已普遍支持 100K–2M token 上下文，那 RAG 是不是过时了？**没有**，原因有三：
...
3. **Lost-in-the-Middle**：Liu et al. 2023, "Lost in the Middle"）实测发现，相关文档放在 context 中间位置时，模型准确率从约 76% 跌到约 54%，**信息位置决定生成质量**——这是架构性的位置偏置。RAG 通过检索 + rerank + top-k 拼接，天然让相关 chunk 排在首尾两侧。
...
工业界的普遍经验：**文档解析质量是 RAG 上游的第一道闸门**。一个被错误切碎成 4 块的表格，比一个错误切碎成 4 段的叙述段落更伤回答——因为表格是结构化信息，结构一断整格全错。（诚实声明：这一条暂未找到可引的定量研究支撑，"伤害大 2–3 倍"等具体倍数同样没有可靠公开出处，引用前请以自建评测集验证。）
...
排名 1 比 100 多 2.6×；排名 1 比 10 多约 15%。**温和的衰减曲线**让"两个检索器都认可"的中等排名比"只有一个第一"更重要。
...
**消融实验**（论文 v3 Table 2，2025-04；完整模型基线：全面性 67.6%、多样性 76.4%）：

| 变体 | 效果 |
|---|---|
| 去掉高层检索（-High） | Agriculture 全面性从 67.6% 降到 **65.2%**（约 2.4 点） |
| 去掉低层检索（-Low） | Agriculture 多样性从 76.4% 降到 **72.0%**（约 4.4 点） |
| 去掉原始文本（-Origin） | 四个数据集**性能未显著下降**，个别还提升 |

前两行说明：**双层检索各有贡献，但去掉任何一层只降 2–4 个点**——LightRAG 的核心收益来自图索引与双层查询的覆盖面，而不是"缺一层就塌"。
...
- **延迟**：每次 LLM 决策约 300–800 ms，多步 1–3 s 起步；
...
| 中 | 100K | 1M | 约 10 GB | 0.001–0.005 美元 | 0.01–0.05 美元 |
| 大 | 10M | 100M | 约 1 TB（+ 量化） | 0.005–0.02 美元 | 0.05–0.3 美元 |
...
**Q4. 文档解析的难点有哪些？**
A. PDF 是"画在白板上的指令"——文字乱序、表格是矢量、公式是图、扫描件要 OCR。复杂版面要"版面分析 + 阅读顺序恢复"。表格和公式要保留结构，HTML/Markdown 形式最佳。**解析质量是 RAG 上游第一道闸门**（表格被切碎比段落被切碎更伤回答）。
...
A. Liu 2023。LLM 对 context 中间位置的内容**利用率显著低于首尾**——U 形曲线。20 篇文档时中间位置准确率从约 76% 跌到约 54%。**RAG 通过 top-k + Rerank 天然规避**（相关 chunk 排首尾）。
...
- **FastAPI + ES + 阿里云百炼** 的招投标 RAG 实战（中型项目，约 50 万 chunk）：推荐参照 LlamaIndex 的 `SubQuestionQueryEngine` + LangGraph 编排多步 agentic 工作流。
