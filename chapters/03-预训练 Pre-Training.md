...
| "过训练"成为新常态 | 2024 年后的模型普遍训练数百到上千 token/参数（LLaMA-3 8B 用 15.6T token ≈ 1950 token/参数，约为 Chinchilla 最优的 100 倍） |
...
$$
2048 \times 8 \times 4096 \times 114 \times 32 \approx 2.45 \times 10^{11} \text{ bytes} \approx 245 \text{ GB}
$$

> **单位口径提醒**：$2.45 \times 10^{11}$ bytes 按十进制（$10^9$）是 **245 GB**，按二进制（$1024^3$）是 **228 GiB**。两个数字都对，只是进制不同；本章其余算例统一用十进制 GB，面试时说出进制即可避免被追问。

**激活显存（245GB）远超模型状态（112GB）**——这就是为什么大模型训练必须开重计算。启用完全重计算（只存每层输入，按 bf16 每元素 2 字节计：$2 \times s \cdot b \cdot h \cdot L \approx 4.29$ GB，与上表口径一致）后激活可降到约 4.3GB。
