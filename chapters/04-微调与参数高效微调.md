...
回顾第 3 章的混合精度训练内存构成：权重 fp16 2B + 梯度 fp16 2B + Adam 状态（fp32 主权重 + 一阶矩 m + 二阶矩 v）= 12B ≈ **总计 16 字节/参数**。
...
- `W₀ ∈ ℝ^{d×d}`：**冻结**，不参与梯度；
- `A ∈ ℝ^{r×d}`：**高斯初始化** N(0, σ²)，σ 小（如 1/√r）保证训练起点信号小；
- `B ∈ ℝ^{d×r}`：**全零初始化**，保证 ΔW = 0 → 训练起点与原模型等价。
...
1. **合并模式**（默认）：把 BA 吸收回 W₀
   ```
       W_new = W₀ + (α/r) · B · A         # O(d²r) 一次性
   ```
...
```
   QLoRA 显存 ≈ 基座 4bit + LoRA bf16 + 激活 + 缓存
              ≈ N × 0.5B  + 4·N·r·d·2B·layer + 激活
```
...
- **公式**：`[P₁, P₂, ..., Pₖ; x₁, ..., xₙ]`，P 是 d_model 维向量，初始可以取自 vocab embedding 的样本。
- **优势**：参数极小（k × d_model），可多任务共享基座、不同 prompt 切换。
...
- **代价**：每层都要存 prefix，参数总量 = L × 2 × k × head_dim；
...
**解答**：前向公式 `y = W₀·x + (α/r)·B·A·x`，合并后 `y = (W₀ + (α/r)·B·A)·x`，合并后矩阵 `W_new = W₀ + (α/r)·B·A` 可一次性算好。下游只剩一次矩阵乘法 `y = W_new·x`，与原推理完全一致，无算子、无分支、无额外访存。
