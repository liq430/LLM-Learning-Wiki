...
$$
\mathcal{L}_{\text{SFT}} \;=\; -\frac{1}{|M|}\sum_{t \in M} \log p_\theta(y_t \mid y_{1:t-1},\, x)
$$
...
LoRA 论文的实验设定：**GPT-3 175B 在 r = 1、仅调 q、v 投影时可训参数约 4.7M**；换到 7B 模型（LLaMA-7B，d=4096、32 层）用 r=8 调 q/v，可训参数为 $2 \times 8 \times 8192 \times 2 \times 32 \approx 4.19\text{M}$（约 0.06%）——同样极小，效果就能逼近全参。
...
**QLoRA（Dettmers et al. 2023）** 让"单张 48GB 显卡微调 65B"成为现实（24GB 消费卡对应 33B）。本节拆解它的三件套。
...
1. **NF4（4-bit NormalFloat）**：信息论最优的 4bit 数据类型。
   ...
   - 配 **双重量化**：对每个分组的缩放因子 s 再做一次量化 → 省约 0.4 bit/参数。

   NF4 的 16 个码点取正态分布分位数的**相邻中点**（k=4 时分母为 $2^k+1=17$；注意 $\Phi^{-1}(0)=-\infty$ 不可计算，所以分位点从 $i/(2^k+1)$ 到 $(i+1)/(2^k+1)$ 取 $i=0,\dots,2^k-1$）：

   $$
   Q_i^{\text{NF4}} \;=\; \tfrac{1}{2}\!\left(\Phi^{-1}\!\left(\tfrac{i}{2^k + 1}\right) + \Phi^{-1}\!\left(\tfrac{i+1}{2^k + 1}\right)\right),\quad i = 0, 1, \dots, 2^k-1\ (k=4)
   $$

   其中 $\Phi^{-1}$ 是标准正态 CDF 的逆函数，码点最后归一化到 $[-1, 1]$ 再乘缩放因子。注意两点：① 码点是**分位数中点而非分位点本身**；② 这个构造**关于 0 非对称**（负侧分位数更密，QLoRA 论文明确说明这是有意为之）。给定权重张量 $X$，分组量化（每组 $b$ 个元素）：
...
| 模型 | 基座 NF4 | LoRA（r=64, all linear，7 矩阵/层 bf16） | 24GB 消费卡 |
|---|---|---|---|
| 7B | 3.5 GB | 约 235 MB | ✅ 充裕 |
| 13B | 6.5 GB | 约 370 MB | ✅ 可行 |
| 33B | 16.5 GB | 约 715 MB | ✅ 紧但够 |
| 65B | 32.5 GB | 约 1.2 GB | ❌ 单卡不行，需 48GB A6000 / A100 80GB |
...
设 $L$ 为 Transformer 层数、$d_{\text{model}}$ 为隐藏维度（每层 K/V 的维度，即 $n_{\text{heads}} \times d_{\text{head}}$）、$k$ 为 prefix 长度：

$$
P_K^{(l)} \in \mathbb{R}^{k \times d_{\text{model}}},\quad P_V^{(l)} \in \mathbb{R}^{k \times d_{\text{model}}},\quad l = 1, \dots, L
$$

参数总量：

$$
|\Theta_{\text{Prefix}}| = \sum_{l=1}^{L} \big(|P_K^{(l)}| + |P_V^{(l)}|\big) \;=\; 2 \cdot L \cdot k \cdot d_{\text{model}}
$$
...
$$
|\Theta_{\text{PT-v2}}| \;=\; 2 \cdot L \cdot k \cdot d_{\text{model}} \;+\; L \cdot \big(d_{\text{hidden}} \cdot k_{\text{in}} + d_{\text{hidden}} \cdot k_{\text{out}}\big)
$$
